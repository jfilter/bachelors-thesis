...
The digital revolution is affecting every part of our life. Also the humanities scholars witness a change in their work life when analog collections are digitized. They have to apply algorithms to organize and analyze huge amount of data. The term ``digital humanitie'' evolved during the last ten years and it can be defined as an intersection between the humanities and information technology. \cite{Svensson2010}.\\

 The information retrieval research group at the computer science department of the Universidad Nacional de Educación a Distancia (UNED) in Madrid cooperates with human scholars to conduct research in digital humanities. In their current project\footnote{\url{http://linhd.uned.es/p/proyecto-dimh}}, they work on historical maps. The maps have been drawn between 1503 and 1805, digitized and are available on the web\footnote{\url{http://www.mcu.es/ccbae/es/consulta/resultados_busqueda.cmd?busq_codsecc=MCAGS}}. The maps were annotated by human scholars. An example of a map is shown in Figure~\ref{figure:map} and its annotation are shown in Figure~\ref{figure:metadata}.\\
...
Because a concept lattice is also a lattice, concept lattices can be represented in Hasse diagrams. The research group computes concept lattices and visualizes them with Hasse diagrams. One result is shown in Figure~\ref{figure:firstVisualizaion}. They are not satisfied  with it because it is hard to see anything on it. The labels are overlapping and the huge amount of edges makes it hard to see anything on it. This is not an individual case and happens when the concept lattice is large. It is my task to create a concept to interactively visualization large concept lattices. The concept should be implemented for a one concept lattice derived from the collection of 

The visualization should convey the information of a concept lattice even if the concept lattice is large. I will develop a concept for visualizing large concept lattice and also implement it for this particular dataset. It should be mentioned, that I am just a part in a big system. It was not my decisoin to apply FCA to this dataset. The evualion will unweigerlich auch zeigen, ob der Gewählte anstaz für diesen datenstz gut is. Regardlich of the outcome of the rightness is my work because it is abstract enouht\\
\begin{figure*}[!ht]
	\centering
...
\caption{Search interfaces offering Typeahead}
\label{figure:fancy2}
\end{figure*}
